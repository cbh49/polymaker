"""Extract WNBA best bets from slate-wide "best bets" articles (one article at a time)."""
...
from wnba_bot.schemas import ArticleRef, DailyArticleFindings, Matchup, TaggedBet
from wnba_bot.summarize import DEFAULT_MODEL, SUMMARIZE_SYSTEM, _response_text, _strip_fences

DAILY_MAX_TOKENS = 1600
...
def _slate_block(matchups: list[Matchup]) -> str:
    lines: list[str] = ["Today's WNBA slate (use these exact team names when tagging bets):"]
    for i, m in enumerate(matchups, start=1):
        lines.append(
            f"{i}. {m.away} @ {m.home} | "
            f"ML {m.away} {m.lines.ml_away} / {m.home} {m.lines.ml_home} | "
            f"Spread {m.away} {m.lines.spread_away} / {m.home} {m.lines.spread_home} | "
...
def _build_daily_prompt(query: str, matchups: list[Matchup], art: ArticleRef) -> str:
    ...
    return f"""Extract ALL recommended WNBA best bets from this article about: "{query}"

{_slate_block(matchups)}

Rules:
- List only concrete recommended plays (moneyline, point spread, over/under game totals).
- Do NOT invent picks. Do NOT include player props.
...
- For spreads, put the spread in line when known and side HOME or AWAY when clear.
