"""Load WNBA matchups from wnba_matchups.json."""
...
from wnba_bot.schemas import Matchup, MatchupLines
...
def matchup_from_row(row: dict[str, Any]) -> Matchup:
    ...
    return Matchup(
        ...
        lines=MatchupLines(
            ...
            spread_away=_str(row, "Team1RunLine"),
            spread_home=_str(row, "Team2RunLine"),
...
def load_matchups(path: str | Path) -> list[Matchup]:
    """Read and normalize wnba_matchups.json into Matchup models."""
    path = Path(path)
