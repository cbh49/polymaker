...
liquidity, volume, reward params, fee schedule, tick size, tokens. Politics
markets are filtered server-side by tag; sports (MLB/WNBA) via `/events` +
`series_slug`.
"""
...
class GammaClient:
    ...
    async def iter_events(
        ...
        *,
        ...
        """Yield raw active/open event dicts for a sports series.

        Uses `series_slug` (e.g. mlb, wnba). Do NOT filter by Gamma `startDate`
        — that is market listing time, not game start. Callers filter on
        `eventDate` / `startTime` instead (`live` / `gameStatus` are unreliable).
        """
        offset = 0
        for _ in range(max_pages):
            params: dict[str, Any] = {
            ...
            if r.status_code in (400, 422):
                log.info("events_pagination_end", series=series_slug, offset=offset,
    ...
    async def market_by_slug(self, slug: str) -> dict[str, Any] | None:
        """Fetch a single active/open market by slug (`GET /markets?slug=`)."""
        try:
            r = await self._client.get(
                "/markets",
            ...
            log.warning("market_by_slug_failed", slug=slug, err=str(exc))
            return None

    async def event_by_slug(self, slug: str) -> dict[str, Any] | None:
        """Fetch a single active/open event by slug (`GET /events?slug=`)."""
        try:
            r = await self._client.get(
                "/events",
                params={"slug": slug, "limit": 1, "active": "true", "closed": "false"},
            ...
            log.warning("event_by_slug_failed", slug=slug, err=str(exc))
            return None
...
def parse_market(raw: dict[str, Any], reward_rates: dict[str, float] | None = None) -> MarketMeta | None:
    ...
    try:
        ...
        event_id = None
        start_time_iso = raw.get("startTime")
        events = raw.get("events") or []
        if events:
            first = events[0] if isinstance(events[0], dict) else {}
            if event_id is None and first.get("id") is not None:
                event_id = str(first.get("id"))
            start_time_iso = start_time_iso or first.get("startTime")
