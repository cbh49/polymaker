...
class CatalogStore:
    ...
    def export_csv(self, path: str | Path, limit: int = 500) -> int:
        ...
        rows = self.top(limit)
        fields = [
            "score", "gap_risk", "reward_pool_per_day", "rebate_pool_per_day", "spread",
        ...
        with open(path, "w", newline="") as fh:
            ...
            for m, sc in rows:
                w.writerow([
                    f"{sc.score:.3f}", f"{sc.gap_risk:.3f}",
...
_META_FIELDS = set(MarketMeta.__dataclass_fields__)


def _load_meta(blob: str) -> MarketMeta:
    d = json.loads(blob)
    d["tokens"] = tuple(TokenMeta(**t) for t in d["tokens"])
    return MarketMeta(**{k: v for k, v in d.items() if k in _META_FIELDS})
