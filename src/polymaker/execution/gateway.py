...
Wraps the synchronous py-clob-client-v2 (EIP-712 signing, pUSD balance
adjustment, tick/fee caching) and offloads blocking network calls to a thread
pool. Supports limit `place()` and taker `market_order()` buys, cancels,
...
class ExecutionGateway:
    ...
    async def connect(self) -> None:
        ...
        if not sec.has_wallet:
            raise RuntimeError(
                "no wallet configured (set POLY_PRIVATE_KEY + POLY_FUNDER, "
                "or PK + BROWSER_ADDRESS, in .env)"
            )
