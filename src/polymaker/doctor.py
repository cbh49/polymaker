...
balance + positions on the funder, and a live market-WS book frame.
"""
...
async def run_doctor(cfg: Config, console: Console) -> bool:
    ...
    check("config loads", True, f"db={cfg.paths.db}")
    check(
        "POLY_PRIVATE_KEY/PK + POLY_FUNDER/BROWSER_ADDRESS set",
        cfg.secrets.has_wallet,
        "put them in .env"
    ...
    async with httpx.AsyncClient(timeout=10) as c:
        ...
        try:
            r = await c.get(
                f"{cfg.wallet.gamma_host}/markets",
    ...
    if cfg.secrets.has_wallet:
        try:
            ...
            check(
                "collateral (pUSD) balance readable",
                bal is not None,
                f"≈{bal:.2f} pUSD on funder {funder[:10]}…" if bal is not None else "check allowances",
            )
            if bal is not None and bal <= 0:
                console.print(
                    "  [yellow]! balance is 0 — deposit USDC and set allowances "
                    "(trade once in the UI)[/yellow]"
                )
            ...
            console.print(
                "  [yellow]! signature-type mismatch? deposit wallets often use "
                "sig_type=3 in config.toml[/yellow]"
            )
    else:
    ...
    token = held_tokens[0] if held_tokens else await _sample_token(cfg)
    if token:
    ...
    if creds is not None:
        passed, detail = await _user_ws_auth(creds, [], cfg.proxy)
        ...
        async with websockets.connect(USER_WS, **kw) as ws:
            ...
            try:
                ...
                if "auth" in low and any(
                    w in low for w in ("fail", "error", "invalid", "unauthor")
                ):
...
async def _sample_token(cfg: Config) -> str | None:
    try:
