...
Usage:
  python scrape_wnba_betting_splits.py
  python scrape_wnba_betting_splits.py --date 2026-08-15
"""
...
DEFAULT_OUT = SCRIPT_DIR / "output" / "wnba_betting_splits.json"
...
def main() -> None:
    ...
    def _already_present(game: dict[str, Any]) -> bool:
        if game.get("matchup") in dk_matchups:
            return True
        key = (str(game.get("away") or "").lower(), str(game.get("home") or "").lower())
        return bool(key[0] and key[1] and key in dk_teams)
