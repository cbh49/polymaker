#!/usr/bin/env python3
"""
Aggregate NFL public betting splits from:
  - DraftKings Network (handle % / bets %; PlayerProps has no NFL)
  - SportsBettingDime (handle % / bets % via nfl-odds; skip Pinnacle)
  ...
  - Polymarket (nfl-YYYY series; moneyline / spread / total)
...
Usage:
  python scrape_nfl_betting_splits.py
  python scrape_nfl_betting_splits.py --date 2026-09-14
"""
...
from nfl_team_map import canonical_abbr, names_match
...
from slate_alignment import NFL_SLATE_WINDOW_DAYS
...
DEFAULT_OUT = SCRIPT_DIR / "output" / "nfl_betting_splits.json"
PAGE_TZ = ZoneInfo("America/Los_Angeles")
WINDOW = NFL_SLATE_WINDOW_DAYS
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Scrape + merge NFL betting splits from DraftKings, SBD, TheSpread, "
    ...
    parser.add_argument(
        "--date",
        default=None,
        help="Slate date YYYY-MM-DD in America/Los_Angeles (default: today Pacific; keeps +7 days)",
    )
    ...
    dk = _scrape_or_empty(
        "DraftKings", lambda: scrape_dk(day=day, headed=args.headed, league="NFL")
    )
    sbd = _scrape_or_empty("SportsBettingDime", lambda: scrape_sbd(day=day, league="NFL"))
    spread = _scrape_or_empty(
        "TheSpread", lambda: scrape_thespread(day=day, headed=args.headed, league="NFL")
    )
    vsin = _scrape_or_empty("VSiN", lambda: scrape_vsin(league="NFL", day=day))
    eva = _scrape_or_empty("EV Analytics", lambda: scrape_eva(league="NFL"))
    covers = _scrape_or_empty("Covers", lambda: scrape_covers(league="NFL"))
    ...
    dk["sources"] = {
        ...
        "pinnacle": {
            "skipped": True,
            "note": "Pinnacle is not published for NFL; sharp findings use DK, SBD, VSiN, TheSpread, EVA, Covers, and Polymarket.",
        },
    }
    ...
    poly = _scrape_or_empty(
        "Polymarket",
        lambda: scrape_polymarket(league="NFL", games=dk.get("games") or [], day=day),
    )
    ...
    print(
        f"Wrote {dk['game_count']} NFL games "
